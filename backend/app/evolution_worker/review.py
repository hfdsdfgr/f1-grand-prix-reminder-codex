...
def main() -> None:
    ...
    parser.add_argument('action', choices=('list', 'publish', 'reject', 'merge'))
    parser.add_argument('event_id', nargs='?')
    parser.add_argument('target_upgrade_id', nargs='?')
    parser.add_argument('--database', default=os.getenv('DATABASE_PATH', 'data/schedules.db'))
    args = parser.parse_args()
    print(json.dumps(review(args.database, args.action, args.event_id, args.target_upgrade_id),
